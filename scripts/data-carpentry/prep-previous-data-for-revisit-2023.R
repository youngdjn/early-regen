...
# turn all column names and values into a paragraph description
for(i in 1:length(d)) {
  d[i, "Description"] =  yaml:: as.yaml(d[i,])
}

d_verbose = d |>
  select(fire, plot_id, plot_type, dist_grn_ALL, starts_with("radius_"), starts_with("vol_"), starts_with("seedl_dens_"), starts_with("dist_grn_"), fire_intens, day_of_burning, shrub_cover, sight_line, Description, lat, lon) |>
  arrange(fire, plot_id)

# Write this for Derek reference in field
write_csv(d_verbose |> select(-Description), file.path(data_dir, "resurvey-prep/previous-plot-summary_verbose.csv"))
...
d_geo = d |>
  mutate(Name = plot_id) |>
  select(Name, Description, lat, lon, plot_type) |>
  arrange(Name)


d_geo = st_as_sf(d_geo, coords = c("lon", "lat"), crs = 4326)

d_geo_sw = d_geo |> filter(plot_type == "seedwall") |> select(-plot_type)
d_geo_core = d_geo |> filter(plot_type == "core") |> select(-plot_type)
d_geo_delayed = d_geo |> filter(plot_type == "delayed") |> select(-plot_type)
